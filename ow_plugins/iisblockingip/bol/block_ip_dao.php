...
class IISBLOCKINGIP_BOL_BlockIpDao extends OW_BaseDao
{
    ...
    public function isLocked()
    {
        ...
        if($user!=null){
            if(time() - (int)OW::getConfig()->getValue('iisblockingip', IISBLOCKINGIP_BOL_Service::EXPIRE_TIME) * 60>$user->getTime()){
                $this->deleteBlockCurrentIp();
                return false;
            }else if($user->getCount()>OW::getConfig()->getValue('iisblockingip', IISBLOCKINGIP_BOL_Service::TRY_COUNT_BLOCK)){
                return true;
            }else{
                return false;
            }
