(function( window, $ ) {'use strict';
    ...
    _methods = {
        ...
        initHashtagEditor: function( slotId )
        {
            ...
            editor.setValue(OW.getLanguageText('admin', 'describe_photo'));
            editor.on('blur', function( editor )
            {
                var value = editor.getValue().trim(), lineCount;
                
                if ( value.length === 0 || value === OW.getLanguageText('admin', 'describe_photo') )
                {
            ...
            editor.on('focus', function( editor )
            {
                ...
                else
                {
                    ...
                    if ( value === OW.getLanguageText('admin', 'describe_photo') )
                    {
                        editor.setValue('');
                    }
                }
        ...
        uploadFile: function( file )
        {
            ...
            if ( _vars.isHTML5 )
            {
                ...
                else
                {
                    ...
                    {
                        OW.error(OW.getLanguageText('admin', 'size_limit', {name: file.name, size: (_vars.maxFileSize / 1048576)}));
                    }
                    else
                    {
                        OW.error(OW.getLanguageText('admin', 'type_error', {name: file.name}));
                    }
            ...
            else
            {
                if ( file.search(/\.(?:jpe?g|png|gif)$/i) !== -1 )
                {
                    ...
                    _elements.dropArea.off('click').on('click', function(){alert(OW.getLanguageText('admin', 'please_wait'))});
                ...
                {
                    OW.error(OW.getLanguageText('admin', 'type_error', {name: file}));
        ...
        requestSuccess: function( jsonStr, slotId )
        {
            ...
            {
                _methods.destroySlot(slotId);
                OW.error(OW.getLanguageText('admin', 'not_all_photos_uploaded'));
            }
        },
    ...
    window.ajaxFileUploader = Object.defineProperties({}, {
        init: { value: function()
        {
            ...
            if ( !_vars.isHTML5 )
            {
                _elements.dropAreaLabel.html(OW.getLanguageText('admin', 'dnd_not_support'));
            }
                ...
                (function()
                {
                    ...
                    if ( _vars.isHTML5 )
                    {
                        eventMap.drop = function( event )
                        {
                            ...
                            _elements.dropAreaLabel.html(OW.getLanguageText('admin', 'dnd_support'));
                        ...
                        eventMap.dragenter = function()
                        {
                            ...
                            _elements.dropAreaLabel.html(OW.getLanguageText('admin', 'drop_here'));
                        };
