...
$weAreTesting = false;
if(OW::getConfig()->configExists("issa","weAreTesting"))
    if(OW::getConfig()->getValue("issa","weAreTesting"))
        $weAreTesting = true;
...
//$log_path = getenv("SNAPSHOT_DIR"). "issa_log.txt";//$log_path = OW_DIR_ROOT .'issa_log.txt';file_put_contents($log_path,"------------------------",FILE_APPEND);
$plugins = BOL_PluginService::getInstance()->findActivePlugins();
foreach ( $plugins as $plugin )
{
    //if($weAreTesting && $plugin->getKey()!=="privacy")   continue;
    ...
    if ( file_exists($pluginRootDir . 'cron.php') )
    {
        ...
        foreach ( $jobs as $job => $interval )
        {
            ...
            $currentStamp = time();
            if ( (( $currentStamp - $runStamp ) >= ( $interval * 60 )) || ($weAreTesting))
            {
